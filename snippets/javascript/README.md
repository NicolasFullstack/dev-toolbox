# Exemples JavaScript

Exemples courts en JavaScript natif, exécutables avec Node.js 20 ou une version ultérieure.

```bash
cd snippets/javascript
npm test
```

La commande utilise le module de test intégré à Node.js : aucune dépendance n'est installée.

## Exemples disponibles

- `products.js` : tableaux, objets, fonctions et absence de mutation ;
- `product-form.js` : DOM, événements et validation de formulaire ;
- `fetch-json.js` : promesses, `async`/`await`, statuts HTTP et erreurs.
